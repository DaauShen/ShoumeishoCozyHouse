...
export default function WikiGalleryAccordion() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      // 1. Lọc ra các item bình thường và item muốn đẩy xuống cuối
      const regularItems = data.filter(
        (item) => item.title !== "Minh họa không chính thức",
      );
      const unofficialItems = data.filter(
        (item) => item.title === "Minh họa không chính thức",
      );

      // 2. Gộp lại với unofficialItems ở dưới cùng
      const sortedData = [...regularItems, ...unofficialItems];

      console.log("Sorted Gallery Data:", sortedData);

      // 3. Map dữ liệu đã gộp
      const sections: ImageSection[] = sortedData.map((item, idx) => ({
        title: item.title,
        value: `section-${idx}`,
        images: item.images,
      }));
  ...
  return (
    ...
    >
      <p className="text-sm text-gray-600 mb-4 text-justify">
        Đây là những minh hoạ chính thức và không chính thức cho Shoumeisho
        Miku. Nhấn vào ảnh để phóng to.
      </p>

      {loading ? (
        // Skeleton dạng block
        <div className="bg-gray-300 animate-pulse rounded-xl h-48" />
      ) : (
        <Accordion
          type="single"
          collapsible
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {section.images.map((img, idx) => (
                    <Dialog
                      key={idx}
                      onOpenChange={(open) =>
                        setSelectedImage(open ? img : null)
                      }
                    >
